...
(defn standard-error
  ...
  [num-observables] (/ 1.04 (math/sqrt num-observables)))
...
(defn item->index-num-leading-zeros-pair
  ...
  [item hash-fn num-observables]
  (let [h (hash-fn item)
        index-bits (log2 num-observables)]
...
(defn- get-bias-correction
  ...
  [^long num-observables]
  {:pre [(power-of-two? num-observables) (>= num-observables 16)]}
  (case num-observables
    16 0.673
    32 0.697
    64 0.709
    (/ 0.7213 (+ 1 (/ 1.079 num-observables)))))

(defn estimate-cardinality
  "Estimates the cardinality of a set using the hyperloglog formula based on the provided vectors of
...
(defn count-distinct
  ...
  [num-observables items]
  (->> items
       (map #(item->index-num-leading-zeros-pair % identity num-observables))
       (reduce (fn [m [idx v]] (assoc m idx (max (m idx) v))) (zero-vec num-observables))
